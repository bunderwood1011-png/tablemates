...
const DEFAULT_SCHEDULE = {
  Monday: 'relaxed', Tuesday: 'busy', Wednesday: 'moderate',
  Thursday: 'relaxed', Friday: 'relaxed', Saturday: 'relaxed', Sunday: 'relaxed'
};
...
function App() {
  ...
  const [members, setMembersRaw] = useState(() => {
    try { const s = localStorage.getItem('tm_members'); return s ? JSON.parse(s) : DEFAULT_MEMBERS; } catch { return DEFAULT_MEMBERS; }
  });

  const [schedule, setSchedule] = useState(() => {
    try { const s = localStorage.getItem('tm_schedule'); return s ? JSON.parse(s) : DEFAULT_SCHEDULE; } catch { return DEFAULT_SCHEDULE; }
  });

  const [meals, setMeals] = useState(() => {
    try { const s = localStorage.getItem('tm_meals'); return s ? JSON.parse(s) : {}; } catch { return {}; }
  });

  const [shoppingList, setShoppingList] = useState(() => {
    try { const s = localStorage.getItem('tm_shopping'); return s ? JSON.parse(s) : null; } catch { return null; }
  });

  const [savedWeeks, setSavedWeeks] = useState(() => {
    try { const s = localStorage.getItem('tm_saved_weeks'); return s ? JSON.parse(s) : []; } catch { return []; }
  });

  const [recipes, setRecipes] = useState(() => {
    try { const s = localStorage.getItem('tm_recipes'); return s ? JSON.parse(s) : []; } catch { return []; }
  });
  ...
  const getSearchResults = () => {
    ...
    recipes.forEach(r => {
      if (r.name.toLowerCase().includes(q)) {
        results.push({ type: 'recipe', name: r.name, sub: 'saved recipe', action: () => { setHighlightedRecipe(r.name); setActiveTab('recipes'); setShowSearch(false); setSearchQuery(''); } });
      }
    });
    ...
    DAYS.forEach(day => {
      if (meals[day] && meals[day].name.toLowerCase().includes(q)) {
        results.push({ type: 'week', name: meals[day].name, sub: 'this week - ' + day, action: () => { setActiveTab('week'); setShowSearch(false); setSearchQuery(''); } });
      }
    });
    ...
    savedWeeks.forEach(week => {
      DAYS.forEach(day => {
        if (week.meals[day] && week.meals[day].name.toLowerCase().includes(q)) {
          results.push({ type: 'past', name: week.meals[day].name, sub: 'week of ' + week.date, action: () => { setActiveTab('pastweeks'); setShowSearch(false); setSearchQuery(''); } });
        }
      });
    });
  ...
  return (
    <div className="app">
      ...
      <div className="header" style={{ marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
          <button
            ...
            style={{ width: '36px', height: '36px', borderRadius: '50%', background: showSearch ? '#1D9E75' : '#e8e8e8', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}
          >
          ...
          <button
            ...
            style={{ width: '36px', height: '36px', borderRadius: '50%', background: activeTab === 'profiles' ? '#1D9E75' : '#e8e8e8', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}
          >
        ...
        {showSearch && (
          <div style={{ marginTop: '12px' }}>
            ...
            {searchResults.length > 0 && (
              <div style={{ background: 'white', borderRadius: '12px', overflow: 'hidden', border: '0.5px solid #e8e8e8' }}>
                {searchResults.map((result, i) => (
                  <div
                    key={i}
                    onClick={result.action}
                    style={{ padding: '10px 14px', borderBottom: i < searchResults.length - 1 ? '0.5px solid #f7f7f5' : 'none', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
                  >
                    <div>
      ...
      <div className="content">
        ...
        {activeTab === 'shopping' && <ShoppingList shoppingList={shoppingList} setShoppingList={setShoppingList} />}
        {activeTab === 'recipes' && <Recipes recipes={recipes} setRecipes={setRecipes} highlightedRecipe={highlightedRecipe} setHighlightedRecipe={setHighlightedRecipe} />}
        {activeTab === 'pastweeks' && <PastWeeks savedWeeks={savedWeeks} setSavedWeeks={setSavedWeeks} onNavigateToRecipe={(meal) => { setHighlightedRecipe(meal.name); setActiveTab('recipes'); }} />}
      </div>
    </div>
  );
}
